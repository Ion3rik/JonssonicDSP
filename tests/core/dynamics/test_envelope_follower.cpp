...
TEST(EnvelopeFollowerTest, PeakTracksStepInput) {
    ...
    env.setAttackTime(1.0_ms, true);    // fast attack
    env.setReleaseTime(100.0_ms, true); // slow release
    env.reset(0.0f);
    float input = 1.0f;
    float last = 0.0f;
    // Attack phase
...
TEST(EnvelopeFollowerTest, RMSMatchesExpected) {
    ...
    env.prepare(1, 48000.0f);
    env.setAttackTime(1.0_ms, true);
    env.setReleaseTime(1.0_ms, true);
    env.reset(0.0f);
    float input = 1.0f;
    float last = 0.0f;
...
TEST(EnvelopeFollowerTest, MultiChannelIndependence) {
    ...
    env.setAttackTime(1.0_ms, true);
    env.setReleaseTime(100.0_ms, true);
    env.reset(0.0f);
